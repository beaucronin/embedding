...
function detectDisplay() {
	if (navigator.getVRDisplays) {
		...
		if (isMobile())
			return DISPLAY_TYPES.MOBILE;
		else
			return DISPLAY_TYPES.DESKTOP;
	}
}
...
function detectInput(displayMode) {
		...
		for (let gamepad of gamepads) {
			if (gamepad && gamepad.pose) {
				if (gamepad.pose.hasPosition) 
					return INPUT_TYPES.VR_6DOF;
				else if (gamepad.pose.hasOrientation)
					return INPUT_TYPES.VR_3DOF;
			}
		}
	}
	...
	if (isMobile()) {
		if (displayMode == DISPLAY_TYPES.VR)
			return INPUT_TYPES.VR_GAZE;
		else 
...
export function detectMode() {
	const displayMode = detectDisplay();
	const inputMode = detectInput(displayMode);
	return { displayMode, inputMode };
}